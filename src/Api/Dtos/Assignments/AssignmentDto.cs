...
public record AssignmentDto(Guid Id, string Title, string Description, DateTime DueDate, decimal MaxScore, DateTime CreatedAt, Guid CourseId, CourseDto? Course)
{
    public static AssignmentDto? FromDomainModel(Assignment assignment)
        => assignment is null ? null : new(assignment.Id.Value, assignment.Title, assignment.Description, assignment.DueDate, assignment.MaxScore, assignment.CreatedAt,
            assignment.CourseId.Value, CourseDto.FromDomainModel(assignment.Course!));
